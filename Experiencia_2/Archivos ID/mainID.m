...
y3 = detrend(y3);
y_PRBS = y3(1:round(length(y3(1:length(y3)-1))/divisiones_periodos)*round(divisiones_periodos));
y_aux = reshape(y_PRBS,divisiones_periodos,[]);
y_PRBS_ant = mean(y_aux);
c = entrada_PRBS(1:length(y_PRBS_ant));
...
figure
semilogx(mag2db(abs(disturbance)));
